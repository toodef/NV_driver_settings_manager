...
unsigned int nv_api::get_value_id_from_value_name( NvU32 setting_id, string const & value_name )
{
   ...
   dword_map_t::iterator it = dwrd_setting_map_.find(setting_id);

   if (it == dwrd_setting_map_.end())
      return 0;

   bimap<string, unsigned int>::left_const_iterator chk_it;

   if ((chk_it = it->second.left.find(value_name)) != it->second.left.end())
...
string nv_api::get_value_name_from_value_id( NvU32 setting_id, unsigned int value_id )
{
   ...
   bimap<string, unsigned int>::right_const_iterator chk_it;

   if ((chk_it = it->second.right.find(value_id)) != it->second.right.end())
...
void nv_api::init_map()
{
   ...
   str_setting_map_[OGL_IMPLICIT_GPU_AFFINITY_ID] = list_of(string((const char *)OGL_IMPLICIT_GPU_AFFINITY_ENV_VAR   ))
   ...
   str_setting_map_[CUDA_EXCLUDED_GPUS_ID] = list_of(string((const char * )CUDA_EXCLUDED_GPUS_NONE));
